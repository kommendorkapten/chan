...
int main(void)
{
	...
	for (int num_threads = 2; num_threads < 10; num_threads += 2) {
		printf("Benchmark (select) with 1 producer, %d consumers\n", 
		       num_threads);
...
void bench_consume(int count)
{
	...
	gettimeofday(&ts, NULL);
	stop = USEC(ts);
	printf("Total number of processed messages: %u\n", thr_msg);
	if (thr_msg != msg_count) 
        {
                printf("ERROR, read %d messaged, expected %u\n", 
                       thr_msg, msg_count);
        }
...
void bench_select1(int count)
{
	...
	for (int i = 0; i < count; i++)
	{
		w[i].c = c;
		w[i].me = i;
		if (pthread_create(&thr[i], NULL, &thr_wrk_select1, (void*) &w[i]))
		{
			perror("bench_consume::pthread_create");
			return;
		}
